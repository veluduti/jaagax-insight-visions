...
export default function SubscriptionManager({ userId }: { userId: string }) {
  ...
  const subscribePremium = async () => {
    ...
    const { error: payErr } = await sb.rpc("decrement_wallet_balance", {
      _user_id: userId, _amount: fee,
      _description: "Premium subscription (monthly)", _reference: `sub:premium:${Date.now()}`,
    });
    if (payErr) { setLoading(false); return toast.error(payErr.message || "Insufficient wallet balance"); }
    ...
    await sb.from("seller_subscriptions").insert({
      user_id: userId, plan_type: "premium", is_active: true, expires_at: expires.toISOString(),
    });
    setLoading(false);
  ...
  return (
    <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.05 }}>
      <Card className="relative overflow-hidden border-emerald-500/20 bg-gradient-to-br from-background via-background to-emerald-500/5">
        <CardContent className="p-5">
          ...
          {isPremium ? (
            <div className="py-3">
              <p className="text-2xl font-bold">Unlimited postings</p>
              <p className="text-xs text-muted-foreground mt-1">Active until next billing cycle</p>
            </div>
          ) : (
            <>
              <div className="flex items-baseline justify-between mb-2">
                <p className="text-sm font-medium">Free posting quota</p>
                <p className="text-sm">
                  <span className="font-bold text-foreground">{status.free_remaining}</span>
                  <span className="text-muted-foreground"> / {status.free_limit} remaining</span>
                </p>
              </div>
            ...
            <Card className="border-border/60">
              <CardContent className="p-4">
                <div className="flex items-start justify-between">
                  <div>
                    <p className="font-semibold">Pay-per-post</p>
            ...
            <Card className="border-emerald-500/40 bg-emerald-500/5">
              <CardContent className="p-4">
                <div className="flex items-start justify-between mb-3">
                  <div>
                    <p className="font-semibold flex items-center gap-1">Premium <Crown className="h-4 w-4 text-emerald-400" /></p>
                    <p className="text-xs text-muted-foreground mt-1">Unlimited posts, priority review, premium badge</p>
                  </div>
      ...
      <Dialog open={agentPrompt} onOpenChange={setAgentPrompt}>
        <DialogContent className="sm:max-w-md">
          <DialogHeader>
            <DialogTitle className="flex items-center gap-2"><Users className="h-5 w-5 text-emerald-400" /> Get leads as an Agent</DialogTitle>
            <DialogDescription>
              You're now on Premium. Switch to an Agent profile to start receiving buyer leads, gain trust score, and rank on the leaderboard.
            </DialogDescription>
          </DialogHeader>
          <ul className="text-sm space-y-2 py-2">
            {["Receive matched buyer leads", "Build a verified agent profile", "Earn XP on the leaderboard"].map((b) => (
              <li key={b} className="flex items-center gap-2"><CheckCircle2 className="h-4 w-4 text-emerald-500" /> {b}</li>
            ))}
          </ul>
          <DialogFooter>
            <Button variant="outline" onClick={() => setAgentPrompt(false)}>Maybe later</Button>
            <Button className="bg-emerald-500 hover:bg-emerald-600 text-white" onClick={() => navigate("/select-profile")}>
              Switch to Agent
            </Button>
          </DialogFooter>
        </DialogContent>
