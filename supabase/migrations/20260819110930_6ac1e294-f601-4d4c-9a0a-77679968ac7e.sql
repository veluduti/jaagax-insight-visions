...
DROP POLICY IF EXISTS "Users manage own agent application" ON public.agent_applications;
CREATE POLICY "Users manage own agent application"
  ON public.agent_applications FOR SELECT TO authenticated
  USING (auth.uid() = user_id OR public.is_admin(auth.uid()));

DROP POLICY IF EXISTS "Users insert own agent application" ON public.agent_applications;
CREATE POLICY "Users insert own agent application"
  ON public.agent_applications FOR INSERT TO authenticated
  WITH CHECK (auth.uid() = user_id);

DROP POLICY IF EXISTS "Users update own pending application" ON public.agent_applications;
